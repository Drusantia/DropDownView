...
open class DropDownView : LinearLayout {
	 ...
	 */
	lateinit var filterContainer: ConstraintLayout
		private set
	...
	var dropdownHintGravity = Gravity.CENTER
		@SuppressLint("RtlHardcoded")
		set(value) {
			field = when (value) {
				Gravity.CENTER_HORIZONTAL, Gravity.CENTER -> Gravity.CENTER
	...
	private fun init(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) {
		...
		try {
			...
			dropdownHintGravity = when (a.getInt(R.styleable.DropDownView_dropdownHint_gravity, Gravity.CENTER)) {
				START -> Gravity.START
				END -> Gravity.END
				else -> Gravity.CENTER
			}
		...
		filterTextView.gravity = dropdownHintGravity
		if (placeholderText?.isNotEmpty() == true) {
			filterTextView.text = placeholderText
			_selectingPosition = -1
		} else _selectingPosition = 0

		filterTextView.gravity = dropdownHintGravity

		//Configure arrow
		if (arrowWidth > -1 || arrowHeight > -1) {
			val arrowLp = filterArrow.layoutParams as ConstraintLayout.LayoutParams
